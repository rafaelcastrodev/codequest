  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <ProgressBar value={questionNumber} max={totalQuestions} variant="secondary" size="sm" />
      </div>
...
function QuizResult({ quiz, correct, stars, onNext, onMap }: QuizResultProps) {
  ...
  return (
    ...
    >
      ...
      <div>
        <h2 className="font-heading font-bold text-2xl text-primary mb-1">Quiz Concluído!</h2>
      ...
      <div className="bg-bg-surface border border-bg-elevated rounded-2xl p-5 space-y-2">
        <p className="text-[#8888AA] font-body text-sm">
          Acertou{' '}
          <span className="text-[#E8E8F0] font-semibold">{correct}</span>
          {' '}de{' '}
          <span className="text-[#E8E8F0] font-semibold">{total}</span>
          {' '}perguntas
        </p>
        <div className="inline-block bg-secondary/20 border border-secondary/30 rounded-xl px-5 py-2">
          <span className="font-heading font-bold text-secondary text-xl">+{quiz.xpReward} XP</span>
        </div>
      </div>
...
export function QuizPage() {
  ...
  if (error || !quiz) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-[#8888AA] font-body">{error ?? 'Quiz não encontrado.'}</p>
      </div>
    );
  }
