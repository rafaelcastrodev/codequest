      ...
      >
        ...
        <ul className="space-y-3 font-body text-sm text-text-main">
          {([
            ...
            { icon: <icons.laptop />, text: 'Resolva exercícios escrevendo código real no editor' },
            { icon: <icons.flask />, text: 'Seu código é compilado e testado automaticamente no navegador' },
            { icon: <icons.bulb />, text: 'Use dicas quando precisar de ajuda (mas tente antes!)' },
            { icon: <icons.star />, text: 'Ganhe estrelas, XP e troféus conforme avança' },
          ] as { icon: ReactNode; text: string }[]).map((item) => (
