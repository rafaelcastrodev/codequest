	...
	return (
		<div className="space-y-4">
			...
			{step.variant === "order-steps" && step.orderSteps && (
				<OrderStepsChallenge
					data={step.orderSteps}
					explanation={step.explanation}
				/>
			)}
			{step.variant === "match-pairs" && step.matchPairs && (
				<MatchPairsChallenge
					data={step.matchPairs}
					explanation={step.explanation}
				/>
			)}
		</div>
...
function FillBlankChallenge({ data, explanation }: FillBlankChallengeProps) {
	...
	const allFilled = data.blanks.every(
		(b) => (answers[b.id] ?? "").trim().length > 0,
	);
	...
	function checkAnswer(blankId: string, input: string): boolean {
		...
		return (
			blank.alternatives?.some((a) => a.toLowerCase() === normalized) ??
			false
		);
	}
	...
	return (
		<div className="space-y-3">
			...
			<RevealFeedback
				...
				isCorrect={data.blanks.every((b) =>
					checkAnswer(b.id, answers[b.id] ?? ""),
				)}
				successLabel="Perfeito!"
				failLabel="Quase!"
			/>
		</div>
	);
...
function OrderStepsChallenge({ data, explanation }: OrderStepsChallengeProps) {
	...
	return (
		<div className="space-y-3">
			...
			<div
				className="flex flex-wrap gap-2 min-h-[2.5rem] p-3 rounded-xl
...
function MatchPairsChallenge({ data, explanation }: MatchPairsChallengeProps) {
	...
	function handleRightClick(rightIdx: number) {
		...
		const alreadyConnectedLeft = [...connections.entries()].find(
			([, r]) => r === rightIdx,
		)?.[0];
	...
	function pairColorForLeft(leftIdx: number): string {
		const connIdx = [...connections.entries()].findIndex(
			([l]) => l === leftIdx,
		);
	...
	return (
		<div className="space-y-3 pt-4">
			<div className="grid grid-cols-2 gap-5">
				...
				<div className="space-y-5">
					{shuffledRight.map((originalIdx) => {
						...
						const isConnected = [...connections.values()].includes(
							originalIdx,
						);
						const colorClass = isConnected
							? pairColorForRight(originalIdx)
							: "";

						let revealClass = "";
						if (revealed && isConnected) {
							const leftIdx = [...connections.entries()].find(
								([, r]) => r === originalIdx,
							)?.[0];
							revealClass =
								leftIdx === originalIdx
			...
			<RevealFeedback
				revealed={revealed}
				explanation={explanation}
				isCorrect={[...connections.entries()].every(
					([l, r]) => l === r,
				)}
				successLabel="Perfeito!"
				failLabel="Quase!"
