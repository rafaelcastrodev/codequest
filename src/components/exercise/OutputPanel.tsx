	...
	return (
		<div className="bg-[#0A0A15] border border-bg-elevated rounded-xl overflow-hidden h-40 flex flex-col">
			<div className="flex items-center gap-2 px-3 py-1.5 border-b border-bg-elevated flex-shrink-0">
				...
				{status === "running" && (
					<span className="ml-auto text-xs text-[#8888AA] font-body animate-pulse">
						executando...
					</span>
				)}
			</div>
			<div className="flex-1 p-3 overflow-y-auto scrollbar-thin">
				...
				{output && (
					<pre
						className={`font-mono text-xs leading-relaxed whitespace-pre-wrap ${status === "passed" ? "text-primary" : "text-[#E8E8F0]"}`}>
						{status === "passed" ? (
							<>
								<icons.checkCircle />{" "}
							</>
